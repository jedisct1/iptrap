...
pub fn frames_to_segments<'a>(frames : &'a [zmq::Message] )
                              -> Vec<&'a [capnp::common::Word]> {    
    let mut result : Vec<&'a [capnp::common::Word]> = Vec::new();

    for frame in frames.iter() {
        unsafe {
            let slice = frame.with_bytes
                (|v| std::raw::Slice { data : v.as_ptr(), len : v.len() / 8 });
...
pub fn send<U:capnp::message::MessageBuilder>(
    socket : &mut zmq::Socket, message : & U) -> Result<(), zmq::Error> {    
